...
export default async function AcademyPage({ params }: Params) {
  ...
  const sequence = [
    { src: "/images/projects/beans-5.webp", caption: beans[4] ?? "", doc: true },
  ...
  return (
    <>
      ...
      >
        <div className="mt-16 grid animate-fade-up gap-4 [animation-delay:240ms] sm:mt-20 sm:grid-cols-3">
          {sequence.map((s, i) => (
            <figure key={s.src}>
              <Media
                src={s.src}
            ...
            return (
              <Reveal key={c.key} delay={(i % 2) * 80}>
                <article className="flex h-full flex-col rounded-3xl bg-surface-2 p-8 sm:p-10">
                  ...
                  <h3 className="mt-6 text-3xl font-semibold tracking-tight text-foreground">{copy.title}</h3>
                  <p className="mt-4 text-lead text-muted">{copy.outcome}</p>
        ...
        <Reveal>
          <div className="grid items-center overflow-hidden rounded-3xl bg-surface lg:grid-cols-2">
            ...
            <div className="p-8 sm:p-12 lg:p-14">
              ...
              <h2 className="mt-3 text-display-sm text-foreground">{t.instructorTitle}</h2>
              <p className="mt-5 text-lead text-muted">{t.instructorBody}</p>
